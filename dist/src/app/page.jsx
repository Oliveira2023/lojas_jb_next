...
export default function Home() {
    const [loja, updateLoja] = useState('Lojas Roland Garros');
    const [grupo, setGrupo] = useState('roland');
    const filteredLojas = FilterStore(grupo);
    const lojasEncontradas = filteredLojas.lojasEncontradas;
    const [isOpen, setIsOpen] = useState(true);
    const node = useRef(null);
    const [teste, updateTeste] = useState(0);
    ...
    // }, [grupo]);
    const updateSelecao = (selecao) => {
        updateLoja(selecao);
    };
    return (<main className="flex min-h-screen flex-col items-center justify-between p-0 ">
...

      {
        ...
        lojasEncontradas.length > 0 ? (lojasEncontradas.map((lojas) => (<div key={lojas.numLoja} className="w-[100%]">
              <CardsLojas gruppo={loja} image={lojas.imageUrl} nome={lojas.nomeLoja} numLoja={lojas.numLoja}/>
            </div>))) : (<div className="w-full text-center bg-red-400 p-2 ">
