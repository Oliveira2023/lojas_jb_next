    ...
    return (<>
            ...
            <div id="hero" className="flex flex-row justify-between ">
                ...
                <div className="m-1 w-[75%] relative">
                    {images.map((image, index) => (<div key={image.Id} className={`${index === activeImgIndex ? "block w-full bg-slate-50" : "hidden"}`}>
                            <Image src={image.src} width={1280} height={844} alt={image.alt} priority={true} placeholder='empty'/>
                        </div>))}
                    <div className="">
                    <Descriptions clickNext={clickNext} clickPrev={clickPrev} activeImgIndex={activeImgIndex}/>
                    </div>
